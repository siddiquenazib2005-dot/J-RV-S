...
ONE FILE — NO EXTERNAL IMPORTS — BULLETPROOF
"""
...
INTENT_MAP = {
    ...
    "translate":  ["translate","translation","in hindi","in english","anuvad","ترجمہ"],
    "summarize":  ["summarize","summary","tldr","brief","short","saaransh"],
    "image":      ["image","photo","picture","analyze image","screenshot","dekho","kya hai is mein"],
    "pdf":        ["pdf","document","analyze this","summarize this file","file mein kya hai"],
}
...
Keep responses clear, structured, and genuinely useful."""
    if memories:
        base += "\n\nThings I remember about the user:\n" + "\n".join([f"- {m}" for m in memories[:10]])
    prompts = {
        "weather":   base + "\nUser asked about weather. Present weather data clearly with emojis.",
        "news":      base + "\nUser wants news. Present as a numbered list of headlines.",
        "search":    base + "\nUser wants web search results. Summarize findings clearly.",
        "wikipedia": base + "\nUser wants Wikipedia info. Give a clear educational summary.",
        "currency":  base + "\nUser wants currency conversion. Show the converted amount clearly.",
        "stock":     base + "\nUser asked about stocks. Present price and change clearly.",
        "code":      base + "\nUser needs coding help. Write clean, commented, working code. Explain briefly.",
        "translate": base + "\nUser wants translation. Give accurate translation with context.",
        "summarize": base + "\nUser wants a summary. Be concise and capture only key points.",
        "image":     base + "\nUser uploaded an image. Describe and analyze it in detail.",
        "pdf":       base + "\nUser uploaded a file. Analyze and summarize the content clearly.",
        "chat":      base + "\nEngage in natural, intelligent conversation. Be helpful and insightful.",
    }
...
def format_tool_result(intent, result):
    if not result or result.get("error"):
        return ""
    if intent == "weather":
        return f"Weather Data — City: {result.get('city')} | Temp: {result.get('temp')}°C | {result.get('description')} | Humidity: {result.get('humidity')}% | Wind: {result.get('wind')} km/h"
    elif intent == "search":
        ...
        return f"Currency: {result.get('amount')} {result.get('from')} = {result.get('converted')} {result.get('to')} (Rate: {result.get('rate')})"
    elif intent == "stock":
...
# ════════════════════════════════════════════════════════════
# SUPABASE — Direct REST API (no library needed)
# ════════════════════════════════════════════════════════════
...
def save_message(session_id, role, content):
    supa("POST", "chat_history", {"session_id": session_id, "role": role, "content": content})

def get_history(session_id, limit=15):
    result = supa("GET", "chat_history", params=f"?session_id=eq.{session_id}&order=created_at&limit={limit}&select=role,content")
    return [{"role": r.get("role"), "content": r.get("content")} for r in result if r.get("role") and r.get("content")]

def save_memory(user_id, memory):
    supa("POST", "user_memory", {"user_id": user_id, "memory": memory})

def get_memories(user_id, limit=15):
    result = supa("GET", "user_memory", params=f"?user_id=eq.{user_id}&order=created_at.desc&limit={limit}&select=memory")
    return [r.get("memory", "") for r in result if r.get("memory")]

def purge_memories(user_id):
    supa("DELETE", "user_memory", params=f"?user_id=eq.{user_id}")

def save_note(user_id, content, title=""):
    supa("POST", "notes", {"user_id": user_id, "title": title, "content": content})

def get_notes(user_id):
    return supa("GET", "notes", params=f"?user_id=eq.{user_id}&order=created_at.desc")

def save_task(user_id, task, priority="normal"):
    supa("POST", "tasks", {"user_id": user_id, "task": task, "priority": priority, "done": False})

def get_tasks(user_id):
    return supa("GET", "tasks", params=f"?user_id=eq.{user_id}&done=eq.false&order=created_at.desc")
...
def wikipedia_search(query):
    try:
        ...
        d = r.json()
        return {"title": d.get("title",""), "summary": d.get("extract","No info found.")[:1000]}
    except Exception as e:
        return {"error": str(e)}
...
def get_stock(symbol):
    try:
        ...
        return {"symbol": symbol.upper(), "price": price, "change": round(price - prev, 2), "currency": meta.get("currency", "USD")}
    except Exception as e:
        return {"error": str(e)}
...
def chat_endpoint():
    try:
        ...
        if intent == "weather":
            ...
            if m:
                tool_result = convert_currency(float(m.group(1)), m.group(2), m.group(3))
...
def mem_save():
    d = request.get_json()
    save_memory(d.get("user_id","default"), d.get("memory",""))
...
def mem_purge():
    purge_memories(request.get_json().get("user_id","default"))
    return jsonify({"success": True})

@app.route("/notes/save", methods=["POST"])
def notes_save():
    d = request.get_json()
    save_note(d.get("user_id","default"), d.get("content",""), d.get("title",""))
    return jsonify({"success": True})
...
def tasks_save():
    d = request.get_json()
    save_task(d.get("user_id","default"), d.get("task",""), d.get("priority","normal"))
    return jsonify({"success": True})
...
def tool_currency():
    return jsonify(convert_currency(
        float(request.args.get("amount",1)),
        request.args.get("from","USD"),
        request.args.get("to","INR")
    ))
